/**
 * ESP32 Audio Recorder with Server Upload - LOCAL WIFI VERSION
 * 
 * Records audio and sends to Docker server for processing
 * Uses local IP address - ESP32 and PC must be on same WiFi
 */
...
// TODO: Replace with YOUR PC's local IP address from ipconfig
// Run "ipconfig" on your PC and find the IPv4 Address (e.g., 192.168.50.123)
const char* SERVER_HOST = "192.168.50.193"; // Fiona's local Wireless LAN adapter WiFi IPv4 address
const int SERVER_PORT = 8000;
const char* WEBSOCKET_PATH = "/ws/esp32/audio/esp32_teammate";
...
bool testServerConnection() {
  ...
  if (httpCode > 0) {
    ...
    Serial.println("   1. Your PC is connected to the same WiFi");
    Serial.println("   2. Docker containers are running (docker ps)");
    Serial.println("   3. SERVER_HOST IP is correct in the code");
    http.end();
    return false;
  }
}
...
void handleRoot() {
  ...
  html += "Server: " + String(SERVER_HOST) + ":" + String(SERVER_PORT) + "<br>";
  html += "ESP32 IP: " + WiFi.localIP().toString() + "<br>";
  html += "Connection: Local WiFi";
  html += "</div>";
...
void setup() {
  ...
  Serial.println("LOCAL WIFI VERSION");
  Serial.println("========================================\n");
  ...
  if (!testServerConnection()) {
    Serial.println("\n⚠️ WARNING: Cannot reach server!");
    Serial.println("Check that:");
    Serial.println("  1. Your PC is on WiFi: " + String(ssid));
    Serial.println("  2. Docker is running: docker ps");
    Serial.println("  3. SERVER_HOST in code matches PC IP");
