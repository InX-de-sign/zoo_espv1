# audio_receiver.py - COMPLETE VERSION with text query support
import asyncio
import json
import base64
import logging
...
class AudioReceiver:
    ...
    async def handle_text_query(self, text: str, client_id: str, websocket: WebSocket):
        """✅ NEW: Handle direct text query without audio"""
    ...
    async def handle_client_with_id(self, websocket: WebSocket, client_id: str, first_message: dict):
        ...
        # Initialize queue and task BEFORE processing
            ...
            while True:
                ...
                if data.get("type") == "audio_chunk":
                    ...
                    if client_id in self.audio_queues:
                        ...
                        logger.warning(f"⚠️ Queue not found for {client_id}, recreating...")
                        self.audio_queues[client_id] = deque(maxlen=300)
                        self.audio_queues[client_id].append(audio_bytes)
                    ...
                    if client_id in self.audio_queues:
                        ...
                        logger.error(f"❌ Cannot process: queue missing for {client_id}")
                
                # ✅ NEW: Handle text queries
                    ...
                    if text.strip():
                        ...
                        logger.warning("⚠️ Empty text query received")
                        await websocket.send_json({
                            "type": "error",
    ...
    async def handle_client(self, websocket: WebSocket, client_id: str):
        ...
        # Initialize queue and task FIRST
        self.audio_queues[client_id] = deque(maxlen=300)
        self.processing_tasks[client_id] = asyncio.create_task(
            self._process_audio_queue(client_id, websocket)
        )
    ...
    async def _process_audio_queue(self, client_id: str, websocket: WebSocket):
        ...
        while True:
            try:
                # Check if client_id exists in queues
                if client_id not in self.audio_queues:
                    logger.warning(f"⚠️ Queue disappeared for {client_id}, exiting processor")
                    break
                ...
                logger.error(f"❌ KeyError in processing: {e}")
                break
            except Exception as e:
    ...
    async def _process_with_openai(self, text: str, client_id: str, audio_ws: WebSocket):
        ...
        if not self.assistant or not self.tts_connections or not self.stream_func:
            ...
            await audio_ws.send_json({
                "type": "error",
                "message": "OpenAI not configured on server"
            })
            return
        ...
        if not tts_ws:
            ...
            await audio_ws.send_json({
                "type": "error",
                "message": "TTS connection not found"
            })
            return
